...
# Download or otherwise extract the PSADT application to the package tools folder
$url = '[[Location]]'
...
if ($process.ExitCode -ne 0) {
    ...
    if ($psAdtExitCodes.ContainsKey($process.ExitCode.ToString())) {
        throw $psAdtExitCodes[$process.ExitCode.ToString()]
    }
    else {
