...
<#
# Launch PSADT with our parameters
$processArgs = @{
    ExeToRun         = (Get-Command powershell).Source
    Statements       = "-File $adtScript -DeploymentType Install -DeployMode $DeployMode"
    WorkingDirectory = $adtFolder
    ValidExitCodes   = @(0,60012)
}
